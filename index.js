...
Multiple vitals shift at once from events (sex: drops 🚿🍴🔥, raises 🚽💧; exertion: drops 😴🚿, raises 🚽💧🧠, etc.).`;
        if (trackOffscreen) {
            step3 += `\nFor off-screen agents track vitals approximately, using the labels from the template in the OUTPUT FORMAT step. Always track off-screen agents’ vitals. If they're asleep, mark WHEN they fell asleep and wake them after 8–12 hours based on prior state; never let them sleep over 12 hours unless comatose. Their lives off-screen must go on: obligations, inner thoughts, places to be, desires, impulses to meet/call/go somewhere, have sex, etc., like real people. Use agent type for this: students rarely sleep before midnight, especially weekends; employed adults usually sleep before midnight for work, though some are irresponsible and lose sleep to online scrolling. Consult the character sheet for realistic traits/habits when tracking off-screen life. STRICTLY AVOID omniscience: off-screen agents have no knowledge of what {{user}} is currently doing.${trackOnscreen ? ' When moving off-screen agent to on-screen, convert their simplified vital values into approximate numbers that correspond to the label, and vice versa.' : ''}`;
        }
