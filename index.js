...
Multiple vitals shift at once from events (sex: drops 🚿🍴🔥, raises 🚽💧; exertion: drops 😴🚿, raises 🚽💧🧠, etc.).`;
        if (trackOffscreen) {
            step3 += `\nFor off-screen agents track vitals approximately, using the labels from the template in the OUTPUT FORMAT step. REMEMBER: you MUST track off-screen character's vitals even when they're not present. Mark the time they fell asleep at (if theyre asleep), and wake them up after 8-12 hours depending on their previous state. AVOID situations where the off-screen agents are asleep for more than 12 hours unless they're in a coma. Their lifes MUST go on - they have obligations, inner thoughts, places to be, desires, impulses to meet/call/go somewhere, to have sex, and so on, just like the real people have it. Don't forget what category of agent this is, too - e.g. students RARELY fall asleep before 12 AM, especially on weekend, grown ups with a job usually ARE asleep before 12 AM so they can wake up and get ready for work next day. Not all of grown ups are responsible, too, depriving themselves of sleep just for a few more hours of mindless online scrolling - so take in the character sheet to analyze how they would realistically operate with their written traits and habits, and implement it when tracking their off-screen life. STRICTLY avoid making them omniscient - if they're OFF-SCREEN, they have no idea what {{user}} is doing, or what are they up to.${trackOnscreen ? ' When moving off-screen agent to on-screen, convert their simplified vital values into approximate numbers that correspond to the label, and vice versa.' : ''}`;
        }
...
► RULES FOR EXISTING RELATIONSHIP TRACKING SYSTEM:
  ...
  IF such system had been detected:
    ...
    - Map its scale to 0–1000 proportionally (e.g. if it uses 0–100 scale, like 'Apathy 76/100', multiply by 10 to make it 1000 like 'Apathy 760/1000').
